<template>
  ...
  <d2-container v-loading.body="loading" class="app-container">
    <el-row :gutter="10">
      <el-col :span="2">
        <el-tag type="primary" class="target-label" color="#36c6d3">收件人
          <svg-icon icon-class="adduser" class="add-user"/>
        </el-tag>
      </el-col>
      <el-col :span="20">
        <multiselect
...
import * as contactsAPI from '@/api/mail/contacts'
import * as mailSendAPI from '@/api/mail/send'
import * as mailDetailAPI from '@/api/mail/inbox'
import { isEmail, isEmpty, getType } from '@/util/validate'
...
export default {
  name: 'mail_send',
  components: { Multiselect, Tinymce },
  data () {
    return {
      loading: true,
      mail: {
  ...
  methods: {
    ...
    addContentHeader(sender, receiveStr, copyStr) {
      const header = `<p><span>------------------------ &nbsp; 原始邮件&nbsp------------------------</span></p>
