    ...
    <div class="app-container calendar-list-container">
        ...
        <div class="filter-container">
            <el-tooltip class="item" effect="dark" content="新增" placement="top-start">
                <el-button  @click="openForm('add')" type="primary" icon="el-icon-plus" class="tool-item filter-item btn-add"></el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="发送选择的联系人" placement="bottom-start">
                <el-button  @click="sendMail()" type="primary" class="tool-item filter-item btn-send">
                    <svg-icon icon-class="send"/>
                </el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="发送全部" placement="top-start">
                <el-button  @click="sendMail('all')" type="primary" class="tool-item filter-item btn-send-to-all">
                    <svg-icon icon-class="send"/>
                </el-button>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="删除选中的联系人" placement="bottom-start">
                <el-button  @click="del" type="danger" icon="el-icon-delete" class="tool-item filter-item btn-del"></el-button>
            </el-tooltip>
            ...
            <el-col :span="8" v-for="(item, index) in list" :key="index" class="card-row">
                <div class="el-card" ref="card" @click="toggleSelect(item, index, $event)">
                    ...
                    <div class="constact-right">
                        <p>
                            <svg-icon icon-class="user" class="user-icon"/>{{item.name}}
                            <el-button  @click="openForm('edit', item, $event)" type="primary" icon="el-icon-edit" size="small" class="card-btn card-btn-edit"></el-button>
                            <el-button  @click="sendMail(null, item, $event)" type="primary" size="small" class="btn-send-mail">
                                <svg-icon icon-class="send" class="send-mail-icon"/>
                            </el-button>
                        </p>
...
<script>
import { fetchList, del, add, edit } from '@/api/mail/contacts'
  ...
  methods: {
    getContacts () {
      ...
      fetchList(this.listQuery).then(res => {
        console.info('contacts:' + JSON.stringify(res.data.contacts))
    ...
    submit () {
      this.$refs.form.validate(valid => {
        if (valid) {
          const handle = this.contactOperation === 'add' ? add : edit
