  ...
  <d2-container>
    <el-row class="tool-bar">
      <el-button-group>
        ...
        <el-button
          ...
          size="small"
        >编辑</el-button>
        <el-button  @click="forward" type="primary" icon="el-icon-share" size="small">转发</el-button>
        <el-button  @click="deleteMail" type="primary" icon="el-icon-delete" size="small">删除</el-button>
        <el-button  @click="initPage" type="primary" size="small">
      ...
      >标记为
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item command="star">
            <svg-icon icon-class="favourite" class="download-icon"/>星标邮件
          </el-dropdown-item>
          <el-dropdown-item v-for="label in labelList" :key="label.id" :command="label.id+''">
            <svg-icon icon-class="label" class="download-icon"/>
            {{label.name}}
          </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
      <el-dropdown split-button type="primary" size="small" menu-align="start">附件
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-for="file in mail.oldFileList" :key="file.name">
            <svg-icon icon-class="download3" class="download-icon"/>
      ...
      <el-dropdown split-button type="primary" size="small" menu-align="start">录音
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-for="audio in mail.oldAudioList" :key="audio.name">
            <svg-icon icon-class="voice4" class="download-icon"/>
    ...
    <div class="mail-info">
      ...
      <div>
        ...
        {{showMailTime | parseTime(('{y}-{m}-{d} {h}:{i}')) }}
      </div>
      <el-row>
        <el-col :span="1">
...
import { fetchDetail, delMail } from '@/api/mail/inbox'
...
export default {
  ...
  methods: {
    ...
    getLabelList () {
      // fetchList().then(res => (this.labelList = res.data.labelList))
    },
    reply (isALL) {
      if (isALL) {
    ...
    toggleStar () {
      toggleStar([this.mail.id]).subscribe({
        next: () => {
          debugger
    ...
    handleMark (labelId) {
      if (labelId === 'star') {
        ...
        markLabel(labelId, [this.mail.id]).subscribe({
          next: () => {
